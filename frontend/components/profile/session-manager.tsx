    ...
    const handleCloseSession = async () => {
        ...
        try {
            ...
            if (result?.signature && result?.pnl !== undefined) {
                ...
                // Submit real PnL to contract — losers' deposits fund winners' payouts
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="p-4 rounded-lg border border-border/50 bg-secondary/20 space-y-2">
                                <div className="flex items-center justify-between text-sm">
                                    ...
                                    <span className="font-mono text-xs truncate max-w-45">{sessionId}</span>
                                </div>
                                <div className="flex items-center justify-between text-sm">
                    ...
                    <div className="bg-card border border-border rounded-xl max-w-md w-full p-6 space-y-4">
                        ...
                        <div className="space-y-4 text-sm">
                            {closeSuccess ? (
                                ...
                                // Has active positions - show info but allow close (backend auto-sells)
                                <>
                                    ...
                                    <div className="p-4 rounded-lg bg-yellow-500/10 border border-yellow-500/20">
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <p className="font-medium text-yellow-500">Positions will be auto-sold</p>
                                        </div>
                                        <p className="text-xs text-muted-foreground">
                                            Closing the session will automatically sell all your open positions at current market prices and settle your PnL.
                                        </p>
                                    </div>
                                    ...
                                    <button
                                        onClick={handleCloseSession}
                                        disabled={isProcessing}
                                        className="w-full py-3 rounded-lg font-mono text-sm bg-orange-600 text-white hover:bg-orange-700 transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
                                    >
                                        {isProcessing ? (
                                            <>
                                                <Loader2 className="h-4 w-4 animate-spin" />
                                                {isSettling || isWaitingSettle ? "Settling on-chain..." : "Auto-selling & closing..."}
                                            </>
                                        ) : (
                                            <>
                                                <XCircle className="h-4 w-4" />
                                                Close Session (Auto-Sell Positions)
                                            </>
                                        )}
                                    </button>
